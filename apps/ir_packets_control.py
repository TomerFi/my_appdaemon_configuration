...
class HandleMqttFan(hass.Hass):
  ...
  def message_arrived(self, event_name, data, kwargs):
    ...
    self.call_service("broadlink/send", host = self.ir_transmitter_ip, packet = ir_packets_manager.get_fan_packet(self.fan_type, self.command))
...
class HandleMqttACUnit(hass.Hass):
  ...
  def send_packet(self, packet):
    self.call_service("broadlink/send", host = self.ir_transmitter_ip, packet = packet)
